...
import { bucketForCategory, getApprovedCorpusGuidance } from "@/lib/lr-approved-corpus-learnings";
...
/** Compact reference of LR categories for reclassification — excludes storage when not applicable */
...
/** Look up LR bucket info by detected lrCode */
...
export async function POST(req: NextRequest) {
  try {
    ...
    let { submissionId, photoId, imageBase64, imageUrl, category, subcategory, milestoneType, expectedLabel, labelDescription } = body;
    ...
    if (!imageBase64 && imageUrl) {
      try {
        ...
        return NextResponse.json({ error: "Failed to fetch imageUrl: " + e?.message }, { status: 400 });
      }
    }

    if (!imageBase64 || !category) {
      return NextResponse.json({ error: "imageBase64/imageUrl and category required" }, { status: 400 });
    }

    // Determine if submission includes storage/battery
    ...
    if (submissionId) {
      ...
      const storagePat = /battery|storage|gateway|transfer.switch|encharge|powerwall|system.controller/i;
      hasStorage = expectedCats.some((c: any) => storagePat.test(c.name || "") || storagePat.test(c.description || ""));
    }
    if (!hasStorage) {
    ...
    const catInfo = await prisma.photoCategory.findFirst({
      where: {
        ...
        subcategoryName: subcategory ?? "Array_Layout",
        milestoneType: milestoneType ?? "Install",
      },
    });

    // Get rejection patterns for this category
    ...
    const corpusGuidance = getApprovedCorpusGuidance(bucketForCategory(category)) || "";
...
CRITICAL — PS8 (STICKER SHEET / STRING DIAGRAM):
- PS8 is a STICKER SHEET (also called a STRING DIAGRAM or STICKER MAP). It documents which specific optimizer/microinverter (or panel/string) is at which position. It is required for SolarEdge and Enphase systems.
- A PS8 can take either form: (a) a PHYSICAL STICKER SHEET — multiple QR/barcode/serial stickers from SolarEdge optimizers or Enphase microinverters affixed to a paper or cardboard layout that maps each sticker to its panel position; OR (b) a STRING / WIRING DIAGRAM — a hand-drawn, sketched, printed, or digitally-drawn schematic that maps the solar strings/panels and their wiring (e.g. rows or boxes representing panels/strings labeled S1, S2, S3, with lines/connections showing how they are wired).
- HAND-DRAWN OR DIGITAL STRING DIAGRAMS QUALIFY — this EXPLICITLY includes a photo, SCREENSHOT, or SMARTPHONE SCREENSHOT of a diagram drawn in a notes/drawing app (e.g. ibisPaint), on paper, on a whiteboard, or in design software.
...
Respond with raw JSON only. No code blocks, no markdown.`;
    ...
    if (!result.ok) {
      console.error("LLM API error:", result.errorText);
    ...
    // Update submission photo if photoId provided
    if (photoId) {
    ...
    // Update submission stats if submissionId provided
    if (submissionId) {
      const photos = await prisma.submissionPhoto.findMany({
        where: { submissionId },
      });

      const analyzed = (photos ?? []).filter((p: any) => p?.confidenceScore != null);
      const avgConf = analyzed?.length > 0
        ? (analyzed ?? []).reduce((s: number, p: any) => s + (p?.confidenceScore ?? 0), 0) / analyzed.length
        : null;
      const issues = (photos ?? []).reduce((s: number, p: any) => s + (p?.issuesFound?.length ?? 0), 0);

      const uniqueCategories = new Set((photos ?? []).filter((p: any) => p?.isAcceptable).map((p: any) => `${p?.category}_${p?.subcategory}`));
      ...
      await prisma.photopackSubmission.update({
        ...
        data: {
          ...
          issuesFound: issues,
          categoriesComplete: uniqueCategories?.size ?? 0,
